...
from local_sandbox import LocalSandboxClient
...
def _get_sandbox_client(config: dict = None) -> LocalSandboxClient:
    """Instantiate and return the LocalSandboxClient."""
    base_url = os.environ.get("SANDBOX_URL", "http://sandbox:8080")
    return LocalSandboxClient(base_url=base_url)


def _get_all_files_sandbox(folder_path: str, sandbox: LocalSandboxClient) -> list[tuple[str, str, int]]:
    """
...
"""
    ...
    try:
        ...
        logger.error(f"Invalid output format when listing files (no JSON array found): {stdout}")
        return []
    except Exception as e:
...
def _execute_code_sandbox(code: str, language: str, sandbox: LocalSandboxClient, timeout: int = 3600) -> tuple[bool, str, str]:
    """
