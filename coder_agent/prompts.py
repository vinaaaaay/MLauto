...
- CRITICAL: The script MUST NOT expect or use any command line arguments (e.g. do not check for "$1" or print usage instructions). Always hardcode the exact python file path "{python_file_path}" directly in the script (e.g., `python "{python_file_path}"`), so it can be executed successfully as a plain zero-argument command `bash execution_script.sh`.
